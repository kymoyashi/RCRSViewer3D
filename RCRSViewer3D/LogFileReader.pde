...
public class LogFileReader extends EntityManager{
  ...
  LogFileReader(int scale, Config config, PApplet applet,InformationManager info){
  ...
  public void readFile(File selection) {
    if (selection == null){
      ...
      try{
        this.log = new FileLogReader(selection,Registry.SYSTEM_REGISTRY);
        super.managerlog = new FileLogReader(selection,Registry.SYSTEM_REGISTRY);
        super.world = StandardWorldModel.createStandardWorldModel(log.getWorldModel(0));
        //super.time = 50;
        super.endTime = log.getMaxTimestep();
        super.changes = new ChangeSet[super.endTime];
        for(int i = 0; i < super.changes.length; ++i){
          if(log.getUpdates(i) == null) super.changes[i] = null;
          else super.changes[i] = log.getUpdates(i).getChangeSet();
        }
