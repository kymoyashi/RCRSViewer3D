...
class CameraParameter {
  ...
  public void camera(PApplet applet)
  {
    ...
    calcSight();
    //applet.camera(centerX,centerY+0.00043, 4000, centerX, centerY, 0, 0, 0, -1);
    if(viewpoint){
      applet.camera((float)vp[0]+centerX, (float)vp[1]+centerY+0.00043, (float)vp[2], centerX, centerY, 0, 0, 0, -1);
    }else{
      applet.camera(centerX,centerY+0.00043,20, (float)vp[0]+centerX, (float)vp[1]+centerY,  (float)vp[2], 0, 0, -1);
    }
  }
  ...
  public boolean setYaw(double y)
  {
    ...
    if (y < 0) yaw = 0;
    else if (y > Math.PI/2) yaw = Math.PI/2;
  ...
  private void calcSight()
  {
    ...
    /*
    if(dirX+dir < Math.PI/2){
      double l = r * Math.tan(dirX+dir) + r * Math.tan(Math.PI-dirX+dir);
      normX1 = (l/2+vp[0]);
      normX2 = normX1-l;
    }else{
      normX1 = r*Math.tan(dir)*-1;
      normX2 = r*Math.tan(dir);
    }
    if(dirY+dir < Math.PI/2){
      double l = r * Math.tan(dirY+dir) + r * Math.tan(Math.PI-dirY+dir);
      normY1 = (l/2+vp[1]);
      normY2 = normY1-l;
    }else{
      normY1 = r*Math.tan(dir)*-1;
      normY2 = r*Math.tan(dir);
    }
    normX1 *= 0.1;
...
*/  
    ...
    //double temp = abs(abs(abs(roll-Math.PI) - Math.PI/2) - Math.PI/4) / Math.PI/4;
    if(roll <= Math.PI*1/4){tempX = roll;}
    else if(roll <= Math.PI*2/4){tempX = Math.PI*1/4 - (roll-Math.PI*1/4);}
    else if(roll <= Math.PI*3/4){tempX = roll-Math.PI*2/4;}
    else if(roll <= Math.PI*4/4){tempX = Math.PI*1/4 - (roll-Math.PI*3/4);}
    else if(roll <= Math.PI*5/4){tempX = roll-Math.PI*4/4;}
    else if(roll <= Math.PI*6/4){tempX = Math.PI*1/4 - (roll-Math.PI*5/4);}
    else if(roll <= Math.PI*7/4){tempX = roll-Math.PI*6/4;}
    else if(roll <= Math.PI*8/4){tempX = Math.PI*1/4 - (roll-Math.PI*7/4);}
  ...
  private float[] calcDirectPoint(float t, float p, double[] vp)
  {
    ...
    result[0] = (float) (vp[2]/Math.tan(dirX + t)) + centerX;
    result[1] = (float) (vp[2]/Math.tan(dirY + p)) + centerY;
    return result;
  }
