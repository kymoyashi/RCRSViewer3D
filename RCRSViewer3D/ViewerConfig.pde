...
class ViewerConfig extends PApplet{
  ...
  {
    if(n >= threshold2L) detail = PLANE;
    else if(n >= thresholdLH) detail = LOW;
    else detail = HIGH;
  }
  ...
  private class Button{
    ...
    public void draw(PApplet p){
      p.pushStyle();
      p.stroke(100);
      if(on)p.fill(180); else p.fill(20);
      p.rect(x,y,this.width,this.height);
      if(on)p.fill(50); else p.fill(200);
      p.text(label,x+width/2,y);
      p.popStyle();
    }
    
    public void push(){
      if(checkMousePos(this.x,this.y,this.width,this.height))on = !on;
    }
