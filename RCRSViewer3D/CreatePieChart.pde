...
public class createChartImage {
  ...
  public createChartImage(double[] data, String t, int time) {
    ...
    if(dataLength > showDisplayData) {
      for(int i = showDisplayData - 1; i >= 0; i--) {
        this.linedata.addValue(data[dataLength - i - 1], t, Integer.toString(time - i));
      }
    } else {
  ...
  public PImage getLineChartPImage(int width, int height, String title, String horizontal, String vertical) {
    chart = ChartFactory.createLineChart(title,horizontal,vertical,linedata,PlotOrientation.VERTICAL,true,false,false);
  ...
  public PImage getPieChartPImage(int width, int height, String title) {
    chart = ChartFactory.createPieChart(title, piedata, false, false, false);
    chart.setBackgroundPaint(Color.GRAY);
    return new PImage(chart.createBufferedImage(width, height));
  }
}
