...
class ViewerConnectionListener extends EntityManager implements ConnectionListener
{
  private TCPConnection connection;
  private boolean started;
  
  private int lastRecordingTime;
  
  private boolean addblockade = true;
  private Set<EntityID> changeSet;
  //private ArrayList<EntityID> entityId = new ArrayList<EntityID>();
  
  public ViewerConnectionListener(int scale, Config config, PApplet applet,InformationManager info){
    super(scale, config, applet,info);
    this.started = false;
    this.lastRecordingTime = 0;
  }
  
  public void connect(String host, int port) throws IOException, ConnectionException{
  ...
  public void messageReceived(Connection c, Message msg){
    if(msg instanceof KVTimestep){
      ...
      if(t.getTime() == 1){
        for(EntityID id : t.getChangeSet().getChangedEntities()){
          if((changes[t.getTime()].getEntityURN(id)) != null){
            if((changes[t.getTime()].getEntityURN(id)).equals("urn:rescuecore2.standard:entity:blockade")){
              Blockade blockade = new Blockade(id);
              blockade.setApexes(((IntArrayProperty)(changes[t.getTime()].getChangedProperty(id, "urn:rescuecore2.standard:property:apexes"))).getValue());
              blockade.setX(((IntProperty)(changes[t.getTime()].getChangedProperty(id, "urn:rescuecore2.standard:property:x"))).getValue());
              blockade.setY(((IntProperty)(changes[t.getTime()].getChangedProperty(id, "urn:rescuecore2.standard:property:y"))).getValue());
              blockade.setPosition(((EntityRefProperty)(changes[t.getTime()].getChangedProperty(id, "urn:rescuecore2.standard:property:position"))).getValue());
              blockade.setRepairCost(((IntProperty)(changes[t.getTime()].getChangedProperty(id, "urn:rescuecore2.standard:property:repaircost"))).getValue());
        ...
        for(EntityID id : t.getChangeSet().getChangedEntities()){
          if((changes[t.getTime()].getEntityURN(id)) != null){
            if((changes[t.getTime()].getEntityURN(id)).equals("urn:rescuecore2.standard:entity:road")){
              super.cBlockadeList.put(id,(((EntityRefListProperty)(changes[t.getTime()].getChangedProperty(id, "urn:rescuecore2.standard:property:blockades"))).getValue()));
            }
          }
        }
      }else if(time > 1){
        ...
        for(EntityID id : t.getChangeSet().getChangedEntities()){
          if((changes[t.getTime()].getEntityURN(id)) != null){
            if((changes[t.getTime()].getEntityURN(id)).equals("urn:rescuecore2.standard:entity:blockade")){
              if((((IntProperty)(changes[t.getTime()].getChangedProperty(id, "urn:rescuecore2.standard:property:repaircost"))).getValue()) != 0){
                Blockade blockade = new Blockade(id);
                blockade.setApexes(((IntArrayProperty)(changes[t.getTime()].getChangedProperty(id, "urn:rescuecore2.standard:property:apexes"))).getValue());
                //blockade.setX(((IntProperty)(changes[t.getTime()].getChangedProperty(id, "urn:rescuecore2.standard:property:x"))).getValue());
                //blockade.setY(((IntProperty)(changes[t.getTime()].getChangedProperty(id, "urn:rescuecore2.standard:property:y"))).getValue());
      ...
      try{
        connection.sendMessage(new VKAcknowledge(ok.getRequestID(),ok.getViewerID()));
        ...
        super.endTime = Integer.parseInt(config.getValue("kernel.timesteps"));
